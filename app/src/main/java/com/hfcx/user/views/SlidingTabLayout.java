...
/**
 * 直接是修改的FlycoTabLayout 里面的类。只是把三角型指示器转换成向下的模式
 */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIndicatorStyle == STYLE_TRIANGLE) {
            if (mIndicatorHeight > 0) {
                ...
                mTrianglePath.reset();
                mTrianglePath.moveTo(paddingLeft + mIndicatorRect.right, height - mIndicatorHeight);
                mTrianglePath.lineTo(paddingLeft + mIndicatorRect.left, height - mIndicatorHeight);
